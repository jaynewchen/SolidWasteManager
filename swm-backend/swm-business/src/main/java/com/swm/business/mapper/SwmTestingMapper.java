...
@Mapper
public interface SwmTestingMapper extends BaseMapper<SwmTesting> {

    @Select("SELECT MAX(testing_no) FROM swm_testing WHERE testing_no LIKE CONCAT(#{prefix}, '%')")
    String selectMaxTestingNoByPrefix(String prefix);
